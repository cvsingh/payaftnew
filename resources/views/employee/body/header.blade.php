...
if ($usertype == 'tsk') {
$allData = Letter::where('tsk_id', $user_id)
->whereNull('so_id')
->get();
$new_inbox = Letter::where('tsk_id', $user_id)
->whereNull('so_id')
->count();
} elseif ($usertype == 'SO') {
$allData = Letter::where('so_id', $user_id)
->whereNull('ar_id')
->get();

$new_inbox = Letter::where('so_id', $user_id)
->whereNull('ar_id')
->count();
} elseif ($usertype == 'AR') {
$allData = Letter::where('ar_id', $user_id)
->whereNull('dr_id')
->get();

$new_inbox = Letter::where('ar_id', $user_id)
->whereNull('dr_id')
->count();
} elseif ($usertype == 'DR') {
$allData = Letter::where('dr_id', $user_id)
->whereNull('jr_id')
->get();

$new_inbox = Letter::where('dr_id', $user_id)
->whereNull('jr_id')
->count();
} elseif ($usertype == 'JR') {
$allData = Letter::where('jr_id', $user_id)
->whereNull('pr_id')
->get();

$new_inbox = Letter::where('jr_id', $user_id)
->whereNull('pr_id')
->count();
} elseif ($usertype == 'PR') {
$allData = Letter::where('pr_id', $user_id)
->whereNull('pr_date')
->get();

$new_inbox = Letter::where('pr_id', $user_id)
->whereNull('pr_date')
->count();
} else {
$allData = [];
...
<header class="header_area">
    ...
    <div class="sidebar_logo">
        <a href="{{ route('employee.dashboard') }}">
            <img src="{{ asset('panel/assets/images/logo.jpeg') }}" alt="" class="img-fluid logo1">
            ...
            <div class="dropdown_wrapper messages_item dropdown-menu dropdown-menu-right">
                ...
                <ul class="dropdown_body nice_scroll scrollbar">
                    ...
                    <li>
                        <a href="#">
                            <div class="text-part">
                                <h6>{{ $letter->letter_no }} <span><i class="far fa-clock"></i>
                                        {{ $letter->letter_date }}</span></h6>
                            </div>
                        </a>
                    </li>
                    @endforeach
        ...
        <li><a data-toggle="dropdown" href="#"><i class="far fa-user"></i>
            ...
            <div class="user_item dropdown-menu dropdown-menu-right">
                <ul>
                    ...
                    <li><a href="#"><span><i class="fas fa-user"></i></span> User
                            Profile</a></li>
                    <li><a href="{{ route('change.password') }} "><span><i class="fas fa-cogs"></i></span> Password Change</a></li>
                    <li>

                        <a href="{{ route('employee.logout') }} "><span><i class="fas fa-unlock-alt"></i></span>
                            Logout</a>
                    </li>
                </ul>
            </div>
